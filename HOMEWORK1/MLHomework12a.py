...
os.makedirs("outputs/HW12a", exist_ok=True)
...
class HousingMLP(nn.Module):
    def __init__(self, input_size, hidden_dims=[256, 128, 64], dropout_rate=0.2):
...
model = HousingMLP(X_train.shape[1]).to(device)
...
plt.legend()
plt.savefig("outputs/HW12a/improved_training_validation_loss.png")
plt.show()
...
np.savetxt("outputs/HW12a/improved_test_predictions.csv", test_predictions, delimiter=",", header="Predicted Prices", comments="")
np.savetxt("outputs/HW12a/improved_test_ground_truth.csv", y_test_actual, delimiter=",", header="Actual Prices", comments="")
