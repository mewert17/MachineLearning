...
# ✅ Modified AlexNet without Dropout for CIFAR-10 and CIFAR-100
...
def plot_results(train_loss, val_loss, val_accuracy, dataset_name):
    ...
    plt.legend()
    plt.savefig(f'models/{dataset_name}_AlexNet_nodropout_Loss.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig(f'models/{dataset_name}_AlexNet_nodropout_Accuracy.png')
    plt.show()

# ✅ Run Training for Both Datasets using the no-dropout model
def main():
    print("🚀 Training AlexNet (nodropout) on CIFAR-10...")
    model_cifar10 = AlexNet_Nodropout(num_classes=10)
    train_loss_cifar10, val_loss_cifar10, val_accuracy_cifar10 = train_model(
        model_cifar10, trainloader_cifar10, testloader_cifar10, num_epochs=50, save_path="alexnet_cifar10_nodropout.pth"
    )
    plot_results(train_loss_cifar10, val_loss_cifar10, val_accuracy_cifar10, "CIFAR-10")

    print("\n🚀 Training AlexNet (nodropout) on CIFAR-100...")
    model_cifar100 = AlexNet_Nodropout(num_classes=100)
    train_loss_cifar100, val_loss_cifar100, val_accuracy_cifar100 = train_model(
        model_cifar100, trainloader_cifar100, testloader_cifar100, num_epochs=50, save_path="alexnet_cifar100_nodropout.pth"
    )
    ...
    print(f"Total number of parameters in AlexNet (nodropout): {total_params}")
